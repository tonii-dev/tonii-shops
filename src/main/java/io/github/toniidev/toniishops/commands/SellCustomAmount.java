...
public class SellCustomAmount implements CommandExecutor {
    ...
    public SellCustomAmount(Plugin plugin){
        this.main = plugin;
    }
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (strings[0].equals(Secret.PASS.getString())) {
            ...
            if (player.getInventory().contains(material)) {
                ...
                player.openInventory(item.getConfirmGUI(new GlobalShopSell(true, amount, player, item.getCumulativeSellPrice(amount)), main));
            } else {
